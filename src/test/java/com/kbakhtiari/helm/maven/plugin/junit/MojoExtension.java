...
public class MojoExtension implements ParameterResolver, BeforeAllCallback, BeforeEachCallback {
  ...
  public void beforeAll(ExtensionContext context) throws Exception {
    ...
    InputStream inputStream =
        AbstractHelmMojo.class.getResourceAsStream("/META-INF/maven/plugin.xml");
    ...
    for (MojoDescriptor mojoDescriptor : plugin.getMojos()) {
      mojoDescriptors.put(
          (Class<? extends AbstractHelmMojo>) Class.forName(mojoDescriptor.getImplementation()),
          mojoDescriptor);
    }
  }
  ...
  public Object resolveParameter(ParameterContext parameterContext, ExtensionContext context)
    ...
    try {
      ...
      Class<? extends AbstractHelmMojo> mojoType =
          (Class<AbstractHelmMojo>) parameterContext.getParameter().getType();
      ...
      for (Parameter parameter : descriptor.getParameters()) {
        ...
        getField(mojoType, parameter.getName())
            .set(mojo, resolve(context, parameter.getDefaultValue()));
      }

      // read mojo values from annotations

      MojoProperty[] mojoProperties =
          ArrayUtils.addAll(
  ...
  private Field getField(Class<? extends AbstractHelmMojo> type, String name) {
    ...
    assertNotNull(
        field, "Field with name '" + name + "' not found at type '" + type.getSimpleName() + "'.");
  ...
  private String resolve(ExtensionContext context, String property) {
    ...
    resolved =
        property.replace(
            "${project.build.directory}", getProjectBuildDirectory(context).toString());
    return resolved;
  }
